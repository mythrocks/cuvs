...
public class BruteForceIndexImpl implements BruteForceIndex {
  ...
  private IndexReference build(
    ...
    try (var resourcesAccessor = resources.access()) {
      ...
      try (var closeableDataMemorySegmentP = allocateRMMSegmentNew(cuvsResources, datasetBytes)) {
  ...
  public SearchResults search(BruteForceQuery cuvsQuery) throws Throwable {
    try (var localArena = Arena.ofConfined()) {
      ...
      try (var resourcesAccessor = cuvsQuery.getResources().access()) {
        ...
        try (var queriesDP = allocateRMMSegmentNew(cuvsResources, queriesBytes);
            var neighborsDP = allocateRMMSegmentNew(cuvsResources, neighborsBytes);
            var distancesDP = allocateRMMSegmentNew(cuvsResources, distanceBytes);
            var prefilterDP =
                prefilterBytes > 0
                    ? allocateRMMSegmentNew(cuvsResources, prefilterBytes)
